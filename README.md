...
You should first configure the operating system's [mail](https://www.interserver.net/tips/kb/linux-mail-command-usage-examples/) client for the user that runs Niffler modules, if you have enabled mail sender for any of the modules through their respective config.json. This is a one-time configuration that is not specific to Niffler.
